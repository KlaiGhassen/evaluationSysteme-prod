...
export class CalendarComponent implements OnInit, AfterViewInit, OnDestroy {
    ...
    ngOnInit(): void {
        ...
        this.eventForm.get('range').valueChanges.subscribe((value) => {
            ...
            this.eventForm
                .get('start')
                .setValue(value.start, { emitEvent: false });
            ...
            if (this.eventForm.get('recurrence').value) {
                ...
                const duration = moment(value.end).diff(
                    moment(value.start),
                    'minutes'
                );
                this.eventForm
                    .get('duration')
                    .setValue(duration, { emitEvent: false });
            ...
            // Otherwise...
                ...
                this.eventForm
                    .get('end')
                    .setValue(value.end, { emitEvent: false });
            }
        });
    ...
    scanQr() {
        this._router.navigate(['calendar/qrScan'], {});
    }
    ...
    private _updateRecurrenceRule(): void {
        ...
        if (
            ...
            (parsedRules['FREQ'] === 'WEEKLY' &&
                parsedRules['BYDAY'].length > 1) ||
        ...
        if (parsedRules['FREQ'] === 'WEEKLY') {
            parsedRules['BYDAY'] = [
                moment(event.start).format('dd').toUpperCase(),
            ];
        }
